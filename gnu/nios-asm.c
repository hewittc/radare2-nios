	...
	if (strncasecmp (*strp, "%lo(", 4) == 0) {
		*strp += 4;
		if (**strp == '-' || ISDIGIT (**strp)) {
			...
			char *k = strchr (*strp, '@');
			if (k != NULL && k[1] == 'h') {
				errmsg = parse_h_address (cd, strp, k, opindex, BFD_RELOC_NIOS_H_LO5, &result_type, &value);
				value >>= 1;
			} else {
				errmsg = cgen_parse_address (cd, strp, opindex, BFD_RELOC_NIOS_LO16_LO5, &result_type, &value);
			}
			if (**strp != ')') {
				return _("missing ')'");
			}
		...
		*strp += 5;
		if (**strp == '-' || ISDIGIT (**strp)) {
			...
			char *k = strchr (*strp, '@');
			if (k != NULL && k[1] == 'h') {
				errmsg = parse_h_address (cd, strp, k, opindex, BFD_RELOC_NIOS_H_XLO5, &result_type, &value);
				value >>= 1;
			} else {
				errmsg = cgen_parse_address (cd, strp, opindex, BFD_RELOC_NIOS_HI16_LO5, &result_type, &value);
			}
			if (**strp != ')') {
				return _("missing ')'");
			}
	...
	if (strncasecmp (*strp, "%hi(", 4) == 0) {
		*strp += 4;
		if (**strp == '-' || ISDIGIT (**strp)) {
			...
			char *k = strchr (*strp, '@');
			if (k != NULL && k[1] == 'h') {
				errmsg = parse_h_address (cd, strp, k, opindex, BFD_RELOC_NIOS_H_HI11, &result_type, &value);
				value >>= 1;
			} else {
				errmsg = cgen_parse_address (cd, strp, opindex, BFD_RELOC_NIOS_LO16_HI11, &result_type, &value);
			}
			if (**strp != ')') {
				return _("missing ')'");
			}
		...
		*strp += 5;
		if (**strp == '-' || ISDIGIT (**strp)) {
			...
			char *k = strchr (*strp, '@');
			if (k != NULL && k[1] == 'h') {
				errmsg = parse_h_address (cd, strp, k, opindex, BFD_RELOC_NIOS_H_XHI11, &result_type, &value);
				value >>= 1;
			} else {
				errmsg = cgen_parse_address (cd, strp, opindex, BFD_RELOC_NIOS_HI16_HI11, &result_type, &value);
			}
			if (**strp != ')') {
				return _("missing ')'");
			}
	...
	while (*syn != 0) {
		...
		if (CGEN_SYNTAX_CHAR_P (*syn)) {
			...
			if (TOLOWER (*str) == TOLOWER (CGEN_SYNTAX_CHAR (*syn))) {
				...
				/* xgettext:c-format */
				sprintf (msg, _("syntax error (expected char `%c', found `%c')"), CGEN_SYNTAX_CHAR (*syn), *str);
				return msg;
			} else {
				/* Ran out of input.  */
				static char msg[80];
				...
				/* xgettext:c-format */
				sprintf (msg, _("syntax error (expected char `%c', found end of instruction)"), CGEN_SYNTAX_CHAR (*syn));
				return msg;
			}
			continue;
		}
...
if(CGEN_INSN_RX(insn) != NULL &&
	...
	continue;
		...
		/*??? 0 is passed for `pc'.  */
		insert_errmsg = CGEN_INSERT_FN (cd, insn) (cd, insn, fields, buf, (bfd_vma)0);
...
#define be_verbose 0
		...
		if (be_verbose) {
			...
			tmp_errmsg = (insert_errmsg? insert_errmsg: parse_errmsg? parse_errmsg
