...
static unsigned int hash_keyword_name(const CGEN_KEYWORD *, const char *, int);
...
cgen_keyword_lookup_name(CGEN_KEYWORD *kt, const char *name) {
	...
	while (ke != NULL) {
		...
		while (*p && (*p == *n || (ISALPHA (*p) && (TOLOWER (*p) == TOLOWER (*n))))) {
			++n, ++p;
		}
...
void cgen_keyword_add(CGEN_KEYWORD *kt, CGEN_KEYWORD_ENTRY *ke) {
	unsigned int hash;
	size_t i;
...
int cgen_insn_count(CGEN_CPU_DESC cd) {
...
int cgen_macro_insn_count(CGEN_CPU_DESC cd) {
...
void cgen_put_insn_value(CGEN_CPU_DESC cd,
	unsigned char *buf,
	int length,
	CGEN_INSN_INT value) {
	int big_p = (cd->insn_endian == CGEN_ENDIAN_BIG);
...
cgen_lookup_insn(CGEN_CPU_DESC cd,
	...
	if (!insn) {
		...
		insn_list = cgen_dis_lookup_insn (cd, (char *)insn_bytes_value, insn_int_value);
		while (insn_list != NULL) {
			...
			if (alias_p
				...
				extract handler.  */
				if ((insn_int_value & CGEN_INSN_BASE_MASK (insn)) == CGEN_INSN_BASE_VALUE (insn)) {
					/*??? 0 is passed for `pc' */
					int elength = CGEN_EXTRACT_FN (cd, insn) (cd, insn, info, insn_int_value, fields, (bfd_vma)0);
					if (elength > 0) {
						/* sanity check */
		...
		/*??? 0 is passed for `pc' */
		length = CGEN_EXTRACT_FN (cd, insn) (cd, insn, info, insn_int_value, fields, (bfd_vma)0);
...
void cgen_get_insn_operands(CGEN_CPU_DESC cd,
	const CGEN_INSN *insn,
...
cgen_lookup_get_insn_operands(CGEN_CPU_DESC cd,
	...
	insn = cgen_lookup_insn (cd, insn, insn_int_value, insn_bytes_value, length, fields, insn != NULL);
	if (!insn) {
		return NULL;
	}
...
void cgen_set_signed_overflow_ok(CGEN_CPU_DESC cd) {
	cd->signed_overflow_ok_p = 1;
}

/* Generate an error message if a signed field in an instruction overflows.  */
void cgen_clear_signed_overflow_ok(CGEN_CPU_DESC cd) {
