...
void cgen_init_parse_operand(CGEN_CPU_DESC cd) {
	...
	(void) (*cd->parse_operand_fn) (cd, CGEN_PARSE_OPERAND_INIT, NULL, 0, 0, NULL, NULL);
}
...
build_asm_hash_table(CGEN_CPU_DESC cd) {
	...
	hash_entry_buf = hash_insn_array (cd, macro_insn_table->init_entries, macro_insn_table->num_init_entries, macro_insn_table->entry_size, asm_hash_table, hash_entry_buf);

	/* Add runtime added insns.
	Later added insns will be prefered over earlier ones.  */

	hash_entry_buf = hash_insn_list (cd, insn_table->new_entries, asm_hash_table, hash_entry_buf);

	/* Add runtime added macro-insns.  */

	hash_insn_list (cd, macro_insn_table->new_entries, asm_hash_table, hash_entry_buf);
...
cgen_parse_keyword(CGEN_CPU_DESC cd ATTRIBUTE_UNUSED,
	...
	while (((p - start) < (int)sizeof (buf)) && *p && (ISALNUM (*p) || *p == '_' || strchr (keyword_table->nonalpha_chars, *p))) {
		++p;
	}
...
cgen_parse_signed_integer(CGEN_CPU_DESC cd,
	...
	errmsg = (*cd->parse_operand_fn) (cd, CGEN_PARSE_OPERAND_INTEGER, strp, opindex, BFD_RELOC_NONE, &result, &value);
	...
	if (!errmsg) {
		...
		if (sizeof (value) > 4 && result == CGEN_PARSE_OPERAND_RESULT_NUMBER && value > 0 && (value & 0x80000000) && ((value >> 31) == 1)) {
			value |= ((bfd_vma)-1) << 31;
		}
...
cgen_parse_address(CGEN_CPU_DESC cd,
	...
	errmsg = (*cd->parse_operand_fn) (cd, CGEN_PARSE_OPERAND_ADDRESS, strp, opindex, opinfo, &result_type, &value);
	/* FIXME: Examine `result'.  */
	if (!errmsg) {
		if (resultp != NULL) {
			*resultp = result_type;
...
cgen_validate_signed_integer(long value, long min, long max) {
	if (value < min || value > max) {
		...
		/* xgettext:c-format */
		sprintf (buf, _("operand out of range (%ld not between %ld and %ld)"), value, min, max);
		return buf;
	}
...
cgen_validate_unsigned_integer(unsigned long value,
	...
	if (value < min || value > max) {
		...
		/* xgettext:c-format */
		sprintf (buf, _("operand out of range (%lu not between %lu and %lu)"), value, min, max);
		return buf;
	}
