...
static int default_print_insn(CGEN_CPU_DESC, bfd_vma, disassemble_info *) ATTRIBUTE_UNUSED;
static int read_insn(CGEN_CPU_DESC, bfd_vma, disassemble_info *, bfd_byte *, int, CGEN_EXTRACT_INFO *, unsigned long *);
...
print_Rbi5(CGEN_CPU_DESC cd, void *dis_info, long value, unsigned int attrs, bfd_vma pc, int length) {
	...
	if (pc > 0) {
		...
		while (insn_list != NULL) {
			...
			handler.  */
			if ((insn_value & CGEN_INSN_BASE_MASK (insn)) == CGEN_INSN_BASE_VALUE (insn)) {
...
void nios_cgen_print_operand(CGEN_CPU_DESC, int, PTR, CGEN_FIELDS *, void const *, bfd_vma, int);
...
void nios_cgen_print_operand(CGEN_CPU_DESC cd,
	int opindex,
	void *xinfo,
	CGEN_FIELDS *fields,
	...
	default:
		/* xgettext:c-format */
		opcodes_error_handler (_("internal error: unrecognized field %d while printing insn"),
			opindex);
		abort ();
	}
}
...
void nios_cgen_init_dis(CGEN_CPU_DESC cd) {
...
print_insn_normal(CGEN_CPU_DESC cd,
	...
	for (syn = CGEN_SYNTAX_STRING (syntax); *syn; ++syn) {
		...
		/* We have an operand.  */
		nios_cgen_print_operand (cd, CGEN_SYNTAX_FIELD (*syn), info, fields, CGEN_INSN_ATTRS (insn), pc, length);
	}
}
...
print_insn(CGEN_CPU_DESC cd,
	...
	basesize = cd->base_insn_bitsize < buflen * 8? cd->base_insn_bitsize: buflen * 8;
	...
	while (insn_list != NULL) {
		...
		if ((unsigned) (CGEN_INSN_BITSIZE (insn) / 8) < buflen &&
			...
			insn_value_cropped = bfd_get_bits (buf, CGEN_INSN_BITSIZE (insn), info->endian == BFD_ENDIAN_BIG);
		} else {
			insn_value_cropped = insn_value;
		}

		if ((insn_value_cropped & CGEN_INSN_BASE_MASK (insn)) == CGEN_INSN_BASE_VALUE (insn)) {
			...
			if (((unsigned)CGEN_INSN_BITSIZE (insn) > cd->base_insn_bitsize) &&
				...
				int rc = read_insn (cd, pc, info, buf, CGEN_INSN_BITSIZE (insn) / 8, &ex_info, &full_insn_value);
				if (rc != 0) {
					return rc;
				}
				length = CGEN_EXTRACT_FN (cd, insn) (cd, insn, &ex_info, full_insn_value, &fields, pc);
			} else {
				length = CGEN_EXTRACT_FN (cd, insn) (cd, insn, &ex_info, insn_value_cropped, &fields, pc);
			}
...
int print_insn_nios(bfd_vma pc, disassemble_info *info) {
...
#define CGEN_BFD_ARCH bfd_arch_nios
	...
	if (cd && (cgen_bitset_compare (isa, prev_isa) != 0 || mach != prev_mach || endian != prev_endian)) {
		cd = 0;
	...
	if (!cd) {
		...
		prev_mach = mach;
		prev_endian = endian;
		cd = nios_cgen_cpu_open (CGEN_CPU_OPEN_ISAS, prev_isa, CGEN_CPU_OPEN_BFDMACH, mach_name, CGEN_CPU_OPEN_ENDIAN, prev_endian, CGEN_CPU_OPEN_END);
		if (!cd) {
			abort ();
		}
