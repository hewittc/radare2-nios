	...
	if (CGEN_BOOL_ATTR (attrs, CGEN_IFLD_SIGN_OPT)) {
		...
		if (!cgen_signed_overflow_ok_p (cd)) {
			...
			if (value < minval || value > maxval) {
				sprintf
					/* xgettext:c-format */
					(errbuf, _("operand out of range (%ld not between %ld and %ld)"), value, minval, maxval);
				return errbuf;
			}
		}
	}
...
insert_insn_normal(CGEN_CPU_DESC cd,
	...
	put_insn_int_value (cd, buffer, cd->base_insn_bitsize, CGEN_FIELDS_BITSIZE (fields), value);

#else

	cgen_put_insn_value (cd, buffer, min ((unsigned)cd->base_insn_bitsize, (unsigned)CGEN_FIELDS_BITSIZE (fields)), value);
	...
	for (syn = CGEN_SYNTAX_STRING (syntax); *syn; ++syn) {
		...
		errmsg = (*cd->insert_operand) (cd, CGEN_SYNTAX_FIELD (*syn), fields, buffer, pc);
		if (errmsg) {
			return errmsg;
		}
	}
...
fill_cache(CGEN_CPU_DESC cd ATTRIBUTE_UNUSED,
	...
	if (bytes) {
		...
		pc += offset;
		status = (*info->read_memory_func) (pc, ex_info->insn_bytes + offset, bytes, info);
...
extract_normal(CGEN_CPU_DESC cd,
	...
	value &= mask;
	/* sign extend? */
	if (CGEN_BOOL_ATTR (attrs, CGEN_IFLD_SIGNED) && (value & (1L << (length - 1)))) {
		value |= ~mask;
	}
...
extract_insn_normal(CGEN_CPU_DESC cd,
	...
	for (syn = CGEN_SYNTAX_STRING (syntax); *syn; ++syn) {
		...
		length = (*cd->extract_operand) (cd, CGEN_SYNTAX_FIELD (*syn), ex_info, insn_value, fields, pc);
		if (length <= 0) {
			return length;
		}
	}
...
const char *nios_cgen_insert_operand(CGEN_CPU_DESC, int, CGEN_FIELDS *, CGEN_INSN_BYTES_PTR, bfd_vma);
...
nios_cgen_insert_operand(CGEN_CPU_DESC cd,
	...
	default:
		/* xgettext:c-format */
		opcodes_error_handler (_("internal error: unrecognized field %d while building insn"),
			opindex);
		abort ();
	}
...
int nios_cgen_extract_operand(CGEN_CPU_DESC, int, CGEN_EXTRACT_INFO *, CGEN_INSN_INT, CGEN_FIELDS *, bfd_vma);
...
int nios_cgen_extract_operand(CGEN_CPU_DESC cd,
	int opindex,
	CGEN_EXTRACT_INFO *ex_info,
	CGEN_INSN_INT insn_value,
	CGEN_FIELDS *fields,
...
int nios_cgen_get_int_operand(CGEN_CPU_DESC cd ATTRIBUTE_UNUSED,
	int opindex,
	...
	default:
		/* xgettext:c-format */
		opcodes_error_handler (_("internal error: unrecognized field %d while getting int operand"),
			opindex);
		abort ();
	}
...
void nios_cgen_set_int_operand(CGEN_CPU_DESC cd ATTRIBUTE_UNUSED,
	int opindex,
	CGEN_FIELDS *fields,
	int value) {
	switch (opindex) {
	...
	default:
		/* xgettext:c-format */
		opcodes_error_handler (_("internal error: unrecognized field %d while setting int operand"),
			opindex);
		abort ();
	}
}

void nios_cgen_set_vma_operand(CGEN_CPU_DESC cd ATTRIBUTE_UNUSED,
	int opindex,
	CGEN_FIELDS *fields,
	bfd_vma value) {
	switch (opindex) {
	...
	default:
		/* xgettext:c-format */
		opcodes_error_handler (_("internal error: unrecognized field %d while setting vma operand"),
			opindex);
		abort ();
	}
}
...
void nios_cgen_init_ibld_table(CGEN_CPU_DESC cd) {
